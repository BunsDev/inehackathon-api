...
class UserController extends PrimateController {
	...
	static async avatar(req, res, next) {
		...
		try {
			...
			if(attachment && attachment.metas?.location) {
				...
				// if the initials is empty, use username
			...
			console.log('Error getting avatar, using fallback:', e);
	...
	static async generateLink(req, res, next) {
		try {
			...
			const data = await UserService.generateLink(id, type);
	...
	static async createExpedient(req, res, next) {
		try {
			const { id } = req.params;

			const data = await UserService.createExpedient(id, req.body);

			res.respond({
				data,
				message: 'Expedient created successfully',
			});
		} catch(e) {
			res.respond({
				status: 400,
				message: 'Error creating expedient: ' + e.message,
			});
		}
	}

	static async getExpedients(req, res, next) {
		try {
			let { id } = req.params;
			if(id === 'me') id = req.user.payload.id;

			const data = await UserService.getExpedients(id);
			...
			res.respond({
				status: 400,
				message: 'Error getting expedients: ' + e.message,
			});
		}
	}

	static async getExpedient(req, res, next) {
		try {
			let { id } = req.params;
			if(id === 'me') id = req.user.payload.id;
			const { uidExpedient } = req.params;

			const data = await UserService.getExpedient(id, uidExpedient);

			res.respond({
				data,
				message: 'Expedient retrieved successfully',
			});
		} catch(e) {
			res.respond({
				status: 400,
				message: 'Error getting expedient: ' + e.message,
			});
		}
	}
}
